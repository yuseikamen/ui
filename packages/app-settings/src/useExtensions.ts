  ...
  try {
    ...
    return {
      ...
      update: async (def: MetadataDef): Promise<boolean> => {
        ...
        try {
          console.log('Calling update from extension ....');
          ...
          // ignore
          console.log('Err while updating from extension is ', error);
        }
...
export default function useExtensions (): Extensions {
  ...
  useEffect((): void => {
    extensions && getKnown(api as any, extensions, trigger).then(setAll);
