...
async function loadOnReady(api: ApiPromise): Promise<State> {
  ...
  registry.setChainProperties(
    createType(registry, 'ChainProperties', { ...properties, ss58Format })
  );
...
export default function Api({
  ...
  const [state, setState] = useState<State>(({ isApiReady: false } as Partial<
    State
  >) as State);
  ...
  return (
    <ApiContext.Provider
      value={{ ...state, api, isApiConnected, isWaitingInjected }}
    >
      {children}
    </ApiContext.Provider>
  );
