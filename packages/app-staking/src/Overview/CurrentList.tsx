...
function filterAccounts (accounts: string[] = [], elected: string[], favorites: string[], without: string[]): AccountExtend[] {
  return accounts
    ...
    .map((accountId): AccountExtend => [
      accountId,
      elected && elected.includes(accountId),
      favorites && favorites.includes(accountId)
    ])
    .sort(sortByFav);
}

function accountsToString (accounts: AccountId[]): string[] {
  return accounts && accounts.map((accountId): string => accountId.toString());
}
  ...
  useEffect((): void => {
    if (stakingOverview) {
      dispatchDetails(validators.map(([address]): AddressDetails => {
        const electedIdx = allElected && allElected.indexOf(address);
