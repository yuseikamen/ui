...
import { AddressMini, AddressSmall, Icon } from '@polkadot/react-components';
...
function expandInfo ({ controllerId, nextSessionIds, stakers, validatorPrefs }: any, myAccounts: string[], withNominations = true): StakingState {
  // const nominators = withNominations && stakers.others
  //   ? stakers.others.map(({ who, value }): [AccountId, Balance] => [who, value.unwrap()])
  //   : [];
  const nominators: any[] = [];
  const stakeTotal = (stakers && stakers.total && !stakers.total.isEmpty && stakers.total.unwrap()) || undefined;
  const stakeOwn = (stakers && stakers.own && !stakers.own.isEmpty && stakers.own.unwrap()) || undefined;
  ...
  return {
    ...
    // isNominatorMe: nominators.some(([who: any]): boolean =>
...
function checkFilter (filter: string, hasNominators: boolean, isElected: boolean, isNominatorMe: boolean, heartbeat?: any): boolean {
...
export default function Address ({ address, className, filter, filterName, hasQueries, heartbeat, isAuthor, isElected, isFavorite, lastBlock, myAccounts, points, setNominators, toggleFavorite, withNominations }: Props): React.ReactElement<Props> | null {
  ...
  const stakingInfo = {controllerId:'0x112', nextSessionIds:'0x98', stakers:"988", validatorPrefs: null}
